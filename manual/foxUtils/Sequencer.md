...
　いくつかのメソッドの紹介
### reset
　イベントを一通り登録した後でこれを実行すると、登録したイベントがキーの順に並び、activeがtrueになる。少なくともこれを1回は実行しないと恒常ループ内においても何も起きないので注意してください。
### pause
　シーケンサーの進行を中断します。elapsedが進まなくなります。bandEventAlwaysをtrueにしている場合、bandEventのactionだけ止めた時のelapsedを元に実行されます。
### start
　pause状態を解除します。
### switchActiveState
　activeとnon-activeの状態を切り替えます。
### update
　これを恒常ループ内で呼び出すことでシーケンサーが進行します。
### isActive
　activeかどうかを示すbooleanを返します。
### deleteSpotEvent, deleteBandEvent
　引数は消したいイベントです。それを渡すと、イベントが削除されます。削除するには一意なイベント名をnameプロパティで登録しておく必要があります。名前の指定は必要なく、イベントを渡すだけです。運用上は、プログラムサイドでイベントを場所などごとに通し番号で管理しておき、消したいときはクリックなどでそれを特定してシーケンサーに渡す、のようなものを想定しています。
### sortSpotEvents
　waiting状態のイベントをキーの順にソートします。動的にイベントを追加する場合、これを明示的に呼び出す必要があります。resetではこれを内部的に呼び出しているため、resetの際にこれを呼び出す必要はありません。なお動的にイベントを追加したとしても、リセットすれば問題なくシーケンサーは再構築されます。
### getElapsed
　経過時間を返します。Discreteの場合は経過フレーム数、Continuousの場合はミリ秒数です。値はリセットで初期化されて0に戻ります。
### getProgress
　経過プログレス（0～1）を返します。ビジュアライズなどのために利用します。
